    ...
    extAPI.storage.local.get(['unitValue'], function (unitResult) {
        ...
        if (uLSaved != false && lSaved != false && hSaved != false && uHSaved != false) {
            if (uLSaved < lSaved) {
                ...
                alertFunc("ERROR: Urgent low alert value must be below the low alert value.");
                return;
            }
            if (uHSaved > hSaved) {
            } else {
                alertFunc("ERROR: Urgent high alert value must be above the high alert value.");
                return;
            }
            if (lSaved < hSaved) {
            } else {
                alertFunc("ERROR: Low alert value must be below the high alert value.");
                return;
            }
